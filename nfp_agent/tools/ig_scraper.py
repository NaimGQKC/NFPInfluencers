...
# (This is now unused as it was for post-filtering, but left for context)
AD_KEYWORDS = [
    "#GrowingUpAnimal",
    "@debeersgroup"
]
...
# --- REVISED FUNCTION 3: GraphQL Post Fetcher (COMMENTED OUT) ---
...
# --- REWRITTEN MAIN FUNCTION (V2.1 - SINGLE SESSION) ---
async def scrape_instagram_target(target_id: int, username: str, skip_stories=False, skip_posts=False):
    """
    SCRAPER MODE: "HYBRID API" (V2.1 - Single Session)
    Uses Playwright for one single session to log in, scrape post links,
    and provide cookies for API calls.
    """
    ...
    async with async_playwright() as p:
        ...
        # --- 3. START SCRAPING (API client + Browser) ---
        # The browser stays open while the API client does its work
        async with httpx.AsyncClient(headers=api_headers, follow_redirects=True, timeout=30.0) as client:
            ...
            # --- 5. GATHER ALL POSTS (COMMENTED OUT) ---
            # if not skip_posts:
            #     logging.info("[Post Scraper] Using existing browser to gather post links...")
            #     
            #     # RE-USE the page object from the login step
            ...
            #     
            #     logging.info("Post links gathered. Proceeding to GraphQL analysis.")
            # 
            #     # --- 6. ANALYZE & GRAPHQL (COMMENTED OUT) ---
            ...
            #             continue
            #         
            #         # Use the API client to fetch post data
        ...
        # --- 7. FINAL CLEANUP ---
        # Now that all API and browser work is done, close the browser.
        await context.close()
        await browser.close()
...
# ... existing code ...
    ...
    # --- Post scraping argument commented out ---
    # parser.add_argument(
    #     "--no-posts",
    #     action="store_true",
    #     help="Do not scrape posts/reels."
    # )
    # --- NEW: Add argument for target ---
    parser.add_argument(
        "target_username",
        type=str,
    ...
    if not target:
        ...
        database.add_target(target_username, "instagram")
        target = database.get_target_by_name(target_username)
    ...
    await scrape_instagram_target(
        ...
        skip_posts=True # Refocused on stories only
    )
