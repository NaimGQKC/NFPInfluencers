    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for i in range(12): # Scrape 12 posts
                ...
                try:
                    # --- NEW: Wait for a *real* video URL ---
                    # We will wait up to 8 seconds for the video element to appear
                    # AND for its 'src' attribute to become a real https:// URL.
                    video_element = page.locator(
                        'div[role="dialog"] div[role="presentation"] video'
                    )

                    # This is a 'wait' function that checks the 'src' attribute
                    await video_element.wait_for(
                        lambda el: el.get_attribute("src") and el.get_attribute("src").startswith("https://"),
                        timeout=8000 # Wait a max of 8 seconds
                    )

                    media_url = await video_element.get_attribute("src")
                    content_type = 'reel'
                    logging.info(f"Successfully waited for and captured Reel URL for {post_id}")

                except Exception as e:
                    # If the 8-second wait fails, it's not a video (or it's a broken post)
                    logging.info(f"No video found (or wait timed out) for {post_id}. Checking for image.")
                    pass

                # If no video was found, look for an image (your existing logic)
                if not media_url:
                    try:
                        img_element = page.locator(
                            'div[role="dialog"] div[role="presentation"] img[style*="object-fit"]'
                        ).first
                        await img_element.wait_for(state="visible", timeout=1000)
                        media_url = await img_element.get_attribute("src", timeout=1000)
                        content_type = 'post'
                    except Exception:
                        logging.warning(f"Could not extract any media URL for {post_id}")                
                # --- 3. ADD GUARDRAIL ---
